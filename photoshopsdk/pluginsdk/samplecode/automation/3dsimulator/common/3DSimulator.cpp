...
{
	OSErr err = noErr;
	Boolean keyEventHappened=KeyICareAboutIsDown();
...
#endif
		if(IsKeyDownInMap(map,kF1Key))
			gRenderState.currentCameraPosition.zAngle += 10;
		else if(IsKeyDownInMap(map,kF2Key))
			gRenderState.currentCameraPosition.zAngle -= 10;
		else if(IsKeyDownInMap(map,kF3Key))
			gRenderState.currentCameraPosition.xAngle += 10;
