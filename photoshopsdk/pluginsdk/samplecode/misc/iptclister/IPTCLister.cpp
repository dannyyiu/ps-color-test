...
bool ScanTIFF(FILE *input)
{
	...
	while (directory != 0)
		...
		fseek( input, directory, SEEK_SET );
		...
		for ( long i = 0; i < entries; i++ )
			...
			fseek(input, directory + 2 + (12 * i), SEEK_SET );
		...
		fseek(input, directory + 2 + (12 * entries), SEEK_SET );
	...
	while ( !feof(input) )
		...
		switch (marker)
			...
			case tJPEG_APPDMarker:
				...
				while ( (tempChar = getc(input)) != NULL)
					{
					tempShort--;
					}
			...
			default:
				...
				fseek( input, tempShort - 2, SEEK_CUR );
				break;
			}
...
bool ScanPhotoshopResources(FILE *input, long length)
{
	...
	while ( (position-start) < length)
		...
		if ( (length - (position - start)) < (4+2+1+4)) {
			...
			fseek( input, start+length, SEEK_SET );
			break;
		}
		...
		unsigned char nameLength = getc(input);
		...
		switch( tagType )
			...
			default:
				...
				fseek( input, tagLength, SEEK_CUR );
				break;
			}
...
static void DoIPTCString(char *descString,FILE *input, unsigned short length)
{
...
void LoadIPTCData(FILE *input, long length, bool includeAll)
{
	...
	if (includeAll)
		...
		tempChar = getc(input);		// copyright boolean
	...
	while ( (position-start) < length)
		...
		unsigned char set = getc(input);
		unsigned short length = GetShort(input,false);
		...
		switch( set )
			...
			case kRecordVersionSet:
				...
				if (tempShort != 2)
					{
					printf("bad IPTC version number!\n");
					return;
					}
				break;
			
			case kObjectNameSet:
				DoIPTCString("Object Name",input,length);
				break;
			...
			case kCategorySet:
				DoIPTCString("Category",input,length);
				break;
			
			case kSpecialInstructionsSet:
				DoIPTCString("Special Instructions",input,length);
				break;
			
			case kDateCreatedSet:
				DoIPTCString("Date Created",input,length);
				break;
			
			case kBylineSet:
				DoIPTCString("Byline",input,length);
				break;
			
			case kBylineTitleSet:
				DoIPTCString("Byline Title",input,length);
				break;
			
			case kCitySet:
				DoIPTCString("City",input,length);
				break;
			
			case kProvinceStateSet:
				DoIPTCString("State/Province",input,length);
				break;
			
			case kCountryNameSet:
				DoIPTCString("Country",input,length);
				break;
			
			case kOriginalTransmissionReferenceSet:
				DoIPTCString("Original Reference",input,length);
				break;
			
			case kHeadlineSet:
				DoIPTCString("Headline",input,length);
				break;
			
			case kCreditSet:
				DoIPTCString("Credit",input,length);
				break;
			
			case kSourceSet:
				DoIPTCString("Source",input,length);
				break;
			
			case kCopyrightNoticeSet:
				DoIPTCString("Copyright Notice",input,length);
				break;
			
			case kCaptionSet:
				DoIPTCString("Caption",input,length);
				break;
			
			case kCaptionWriterSet:
				DoIPTCString("Caption Writer",input,length);
				break;
