...
public:
	PropetizerDialog() : PIDialog(), fBigNudgeV(0.0), fBigNudgeH(0.0), 
		...
		fSliceIndex(0), fiBigNudgeV(0), fiBigNudgeH(0), fiRulerOriginV(0), 
		fiRulerOriginH(0), fiGridMajor(0), fiGridMinor(0), fiWatchSuspension(0),
		fiCopyright(0), fiURL(0), fiWatermark(0), fiSliceID(0), fiSliceIndex(0), fiXMP(0)
		{}
	~PropetizerDialog() {}
...
vector<string> GetInterfaceColors(void)
{
	...
	for (int8 color = kPIInterfaceButtonUpFill; color <= kPIInterfaceRed; color++)
		if (!PIGetInterfaceColor(color, iColor))
		{
...
int32 PropetizerData::DoUI(void)
{
    bool writeEndOfLine = true;
    
    Logger logger("Propetizer");
    
    logger.Write("fBigNudgeV: ");
    logger.Write(fBigNudgeV, writeEndOfLine);

    logger.Write("fBigNudgeH: ");
    logger.Write(fBigNudgeH, writeEndOfLine);
    
    logger.Write("fRulerOriginV: ");
    logger.Write(fRulerOriginV, writeEndOfLine);

    logger.Write("fRulerOriginH: ");
    logger.Write(fRulerOriginH, writeEndOfLine);
	
    logger.Write("fGridMajor: ");
    logger.Write(fGridMajor, writeEndOfLine);

    logger.Write("fGridMinor: ");
    logger.Write(fGridMinor, writeEndOfLine);
    
    logger.Write("fWatchSuspension: ");
    logger.Write(fWatchSuspension, writeEndOfLine);
    
    logger.Write("fCopyright: ");
    logger.Write(fCopyright, writeEndOfLine);
    
    logger.Write("fURL: ");
    logger.Write(fURL, writeEndOfLine);
    
    logger.Write("fWatermark: ");
    logger.Write(fWatermark, writeEndOfLine);
    
    logger.Write("fSliceID: ");
    logger.Write(fSliceID, writeEndOfLine);
    
    logger.Write("fSliceIndex: ");
    logger.Write(fSliceIndex, writeEndOfLine);
    
    logger.Write("fXMP: ");
    logger.Write(fXMP, writeEndOfLine);
