...
static size_t StringLength3D(wchar_t *string)
{
	if(!string)
		return 0;
...
static size_t StringLength3D(uint16 *string)
{
	if(!string)
		return 0;
...
static void StringCopy3D(uint16 *destString,wchar_t *sourceString,size_t destMaxLength)
{
	if(!destString || !sourceString)
		return;
	
	size_t strLen=StringLength3D(sourceString);

	if (strLen >= destMaxLength) 
		return;
...
static void StringCopy3D(uint16 *destString,uint16 *sourceString,size_t destMaxLength)
{
	if(!destString || !sourceString)
		return;
...
static void StringCopy3D(char *destString,uint16 *sourceString,size_t destMaxLength)
{
	if(!destString || !sourceString)
		return;
...
static void StringAppend3D(uint16 *destString,char *sourceString, size_t destMaxLength)
{
	if(!destString || !sourceString)
		return;
		
	size_t strLenToAppend=strlen(sourceString);
	size_t strLenInitial=StringLength3D(destString);

	if ((strLenToAppend + strLenInitial) >= destMaxLength) 
		return;
...
void DoStart(void)
{
	...
	strncpy (theNewLight->name, "My New Light", kMaxNodeNameLength-1);
	...
	// Add an image
	strncpy((char*)theMaterial->maps[PI3DDiffuseMap].map,"/Users/falco/test.jpg", 2047);
	theMaterial->maps[PI3DDiffuseMap].map[2047] = '\0';
	...
	for(uint32 index=0;index < theLayer.textureList.length; index++)
		{
		...
		StringCopy3D(textureList.textureNames[index],theLayer.textureList.textureNames[index],256 * sizeof(uint16));
		StringCopy3D(textureList.texturePaths[index],theLayer.textureList.texturePaths[index],256 * sizeof(uint16));
	...
	StringCopy3D(textureList.textureNames[newIndex],(wchar_t*)L"/Users/falco/test.jpg",256 * sizeof(uint16));
	StringCopy3D(textureList.texturePaths[newIndex],(wchar_t*)L"/Users/falco/test.jpg",256 * sizeof(uint16));
